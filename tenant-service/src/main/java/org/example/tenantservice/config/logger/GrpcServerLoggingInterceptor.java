...
public class GrpcServerLoggingInterceptor implements ServerInterceptor {
    ...
    /** Intercepts incoming gRPC calls to log request and response details */
    @Override
        ...
        ServerCall<ReqT, RespT> loggingCall =
                new ForwardingServerCall.SimpleForwardingServerCall<ReqT, RespT>(call) {
                    ...
                    public void close(Status status, Metadata trailers) {
                        ...
                        if (status.isOk()) {
                            log.info(
                                    "[COMPLETED gRPC SERVER] Method: {} | Status: OK | Duration: {}ms",
                                    methodName,
                                    duration);
                        } else {
                            log.error(
                                    "[COMPLETED gRPC SERVER] Method: {} | Status: {} | Description: {} | Duration: {}ms",
                                    methodName,
                                    status.getCode(),
